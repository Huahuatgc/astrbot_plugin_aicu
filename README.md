...
这是一个用于 AstrBot 的插件，可以通过 UID 查询 AICU 数据，生成包含 B 站用户资料、设备型号及近期评论统计的图片报表。
...
​设备分析：展示用户评论时使用的设备型号和曾用名。//内容来自aicu.cc，不保证真实性和实时性。
​活跃统计：抓取近期 100 条评论，统计活跃时段。
​精美报表：使用 Playwright + Jinja2 生成 HTML 并渲染为图片发送。

​🛠️ 安装与依赖
​使用前请确保在控制台安装了必要的 Python依赖
...
​1. 获取 Cookie
​PC 端：登录 aicu.cc，按 F12 打开开发者工具 -> 点击 网络 (Network) -> 刷新页面 -> 点击任意请求 -> 复制请求头中的 Cookie 值。
​移动端：可以使用 Via 浏览器 等支持查看资源的浏览器获取 Cookie。
